...
template <> struct py_converter<solve_parameters_t> {
 static PyObject *c2py(solve_parameters_t const & x) {
  ...
  PyDict_SetItemString( d, "measure_g_pp_tau"      , convert_to_python(x.measure_g_pp_tau));
 ...
 static solve_parameters_t py2c(PyObject *dic) {
  ...
  _get_optional(dic, "measure_g_pp_tau"      , res.measure_g_pp_tau         ,false);
 ...
 static bool is_convertible(PyObject *dic, bool raise_exception) {
  ...
  std::vector<std::string> ks, all_keys = {"h_int","n_cycles","partition_method","quantum_numbers","length_cycle","n_warmup_cycles","random_seed","random_name","max_time","verbosity","move_shift","move_double","use_trace_estimator","measure_g_tau","measure_g_l","measure_pert_order","measure_density_matrix","measure_g_pp_tau","use_norm_as_weight","performance_analysis","proposal_prob","move_global","move_global_prob","imag_threshold"};
  ...
  _check_optional <bool                                >(dic, fs, err, "measure_g_pp_tau"      , "bool");
