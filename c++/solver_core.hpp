...
class solver_core {
 ...
 std::vector<block_gf<imtime>> _G_pp_tau;       // Pseudoparticle Green's function container
 ...
 /// Accumulated :math:`G(\tau)` of pseudoparticles in imaginary time.
 block_gf_view<imtime> get_G_pp_tau(int i) {
   if (i >= _G_pp_tau.size()) {TRIQS_RUNTIME_ERROR << "state index i out of range";}
   return _G_pp_tau[i];
 }
